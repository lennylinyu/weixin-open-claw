...
import { MessageItem, MessageState, MessageType, UploadMediaType } from './../types';
...
import { createFileMessage, createImageMessage, createTextMessage, createVideoMessage, sendMessage } from '../message/message.create';
import { parseMessage, ParseResult } from '../message/message.parse';
import { createFileInfo, createUploadURL, uploadFileToCND } from '../message/message.upload';
 ...
 */
type IMessageResult = GetUpdatesResp & { results?: (Promise<ParseResult> | ParseResult)[] };
...
export default class Client {
    ...
    sendMessage(message: SendMessageReq) {
        return sendMessage({ baseUrl: this.$apiBaseURL, message, contextToken: this.$botToken }).then(() => {
     ...
     */
    async sendMessageByFile(file: File | Buffer | string, fileType: (typeof UploadMediaType)[keyof typeof UploadMediaType] = UploadMediaType.FILE) {
        // 创建文件上传信息 / Create file upload info
        const info = await createFileInfo(file, fileType)

        // 获取 CDN 上传 URL / Get CDN upload URL
        const uploadURL = await createUploadURL({ baseUrl: this.$apiBaseURL, token: this.$botToken, userID: this.$userID, fileInfo: info })
        info.cdnURL = uploadURL.upload_param

        // 上传加密文件到 CDN / Upload encrypted file to CDN
        info.downdownURL = await uploadFileToCND({ baseUrl: getCdnUrl(), info }) ?? ''
