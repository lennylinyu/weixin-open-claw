 ...
 */
export type ParseResult = string | Uint8ClampedArray | Blob | Buffer | { key?: Buffer<ArrayBufferLike>, url: string };

/**
 * 消息类型解析器缓存（按 MessageItemType 映射到对应的解析函数）
 * Message type parser cache (maps MessageItemType to corresponding parse function)
 */
const CACHE_PARSE = new Map<number, (item: MessageItem) => Promise<ParseResult> | ParseResult>();
 ...
 */
export function parseMessage(messageItem: MessageItem | MessageItem[]): (Promise<ParseResult> | ParseResult)[] {
